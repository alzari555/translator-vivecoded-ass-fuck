...
* ✂️ **Motor de Troceado Inteligente (Chunking):** Permite traducir textos enormes respetando el contexto limitado de tu LLM. Puedes configurar manualmente los límites de caracteres por bloque y los tokens de respuesta esperados. Además, el algoritmo corta de forma natural buscando signos de puntuación en el texto para no romper palabras y une internamente los fragmentos traducidos sin desconfigurar tus párrafos.
* 🔗 **Coherencia por Superposición (Overlap):** El motor inyecta un porcentaje de la traducción del bloque anterior como "contexto" en el siguiente bloque para mantener el sentido gramatical, alineándose dinámicamente al primer espacio limpio para nunca enviar palabras rotas a la IA.
* 🧠 **Soporte de Razonamiento Automatizado:** Inyecta automáticamente la directiva `reasoning_effort: "low"` en el backend (cuando está soportado). El sistema, por defecto y de manera invisible, oculta la cadena de pensamiento (etiquetas `<think>`) típica de modelos como DeepSeek-R1 mientras se genera la traducción.
